function q=QuestRecomputeGauss(q)
% q=QuestRecomputeGauss(q)
%
...
%
% modified, 2011-09-05 10:07:24 ban
% change psychometric function from Weilbull to cumulative Gaussian

if nargin~=1
    error('Usage: q=QuestRecomputeGauss(q)')
end
if length(q)>1
    for i=1:length(q(:))
...
% ======================================================================
%q.p2=q.delta*q.gamma+(1-q.delta)*(1-(1-q.gamma)*exp(-10.^(q.beta*q.x2))); % Weibull function
q.p2=0.5*q.delta+0.5*(1-q.delta).*(1+erf((q.x2-q.gamma)./sqrt(2)./q.beta)); % Cumulative Gaussian
% ======================================================================
...
% ======================================================================
%q.p2=q.delta*q.gamma+(1-q.delta)*(1-(1-q.gamma)*exp(-10.^(q.beta*(q.x2+q.xThreshold)))); % Weibull function
q.p2=0.5*q.delta+0.5*(1-q.delta).*(1+erf((q.x2+q.xThreshold-q.gamma)./sqrt(2)./q.beta)); % Cumulative Gaussian
% ======================================================================
...
%%%%% TEMPORAL SETTING, PLEASE COMMNET OUT THE LINE BELOW IF NOT REQUIRED THIS ADJUSTMENT %%%%%
if ~isfield(q,'trialCount'), q.trialCount=50; end
%%%%% TEMPORAL SETTING, PLEASE COMMNET OUT THE LINE ABOVE IF NOT REQUIRED THIS ADJUSTMENT %%%%%
