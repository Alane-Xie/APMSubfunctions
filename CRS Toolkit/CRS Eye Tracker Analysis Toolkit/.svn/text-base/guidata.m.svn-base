...
%Modified GUIDATA for use in UISUBGUI.
...
%Start - Lior cohen addin for UISUBGUI
%token - '::subgui_tag::'
tag=get(h,'tag');
[s,f,t]=regexp(tag,'::(.*)::');
if isempty(t)    
    ...
    subgui=1;
    ix=t{:};
    subgui_tag=tag(ix(1):ix(2));
end

if subgui
    if nargin == 1 % GET
        data = getappdata(fig, PROP_NAME);
        data=data.(subgui_tag);
    else % (nargin == 2) SET
        data=getappdata(fig, PROP_NAME);
        data.(subgui_tag)=data_in;
        setappdata(fig, PROP_NAME,data);
    end
    return;
end
%End - Lior cohen addin for UISUBGUI


%guidata original code
if nargin == 1 % GET
  data = getappdata(fig, PROP_NAME);
else % (nargin == 2) SET
