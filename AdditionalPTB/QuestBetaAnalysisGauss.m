...
if nargin<1 || nargin>2
    error('Usage: QuestBetaAnalysisGauss(q,[fid])')
end
if nargin<2
    fid=1;
end
...
for i=1:length(q(:))
  [betaEstimate(i),thresholdEstimate(i)]=QuestBetaAnalysis1(q(i),fid);
end
return

function [betaEstimate,thresholdEstimate]=QuestBetaAnalysis1(q,fid)
for i=1:9 %1:16 % to fit cumulative gaussian
    q2(i)=q;
    q2(i).beta=2^(i/4);
    q2(i).dim=250;
    q2(i).grain=0.02;
end
qq=QuestRecomputeGauss(q2);
