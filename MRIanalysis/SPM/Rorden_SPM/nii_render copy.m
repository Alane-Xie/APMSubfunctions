function nii_render (P, PreserveCSF, Normalize, Thresh);
...
%    spm_jobman('run',matlabbatch);
    if Normalize
        ...
        if PreserveCSF
            extractsub(Thresh,fullfile(pth,['wm',  nam, ext]),fullfile(pth,['wc1',  nam, ext]),fullfile(pth,['wc2',  nam, ext]),fullfile(pth,['wc3',  nam, ext]));
        else
            extractsub(Thresh,fullfile(pth,['wm',  nam, ext]),fullfile(pth,['wc1',  nam, ext]),fullfile(pth,['wc2',  nam, ext]),'');            
        end;
    else %if normalized else native space
        if PreserveCSF
            extractsub(Thresh,fullfile(pth,['m',  nam, ext]),fullfile(pth,['c1',  nam, ext]),fullfile(pth,['c2',  nam, ext]),fullfile(pth,['c3',  nam, ext]));
        else
            extractsub(Thresh,fullfile(pth,['m',  nam, ext]),fullfile(pth,['c1',  nam, ext]),fullfile(pth,['c2',  nam, ext]),'');            
        end;    end;
end; %for each image...
%end main function
...
function extractsub(thresh, t1, c1, c2, c3)   
...
if thresh <= 0
    ...
    %we could dilate by one voxel...
    %mask = dilatesub(mask, 1);
    spm_write_vol(mi,mask); %save binary mask
    spm_smooth(mi,mi,1,0); %feather the edges with 1mm FWHM
    mask = spm_read_vols(mi);
    delete (mi.fname); %this is 32-bit, but we will next save as 16-bit: SPM overwrites rather than erases images
    m=m.*mask;
end;
