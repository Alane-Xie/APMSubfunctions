function handles = barweb(barvalues, lowerlim, upperlim, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend)
...
if nargin < 11
    fprintf('Insufficient input arguments provided: reverting to default settings.\n');
    width = 1;
...
if size(barvalues,1) ~= size(lowerlim,1) || size(barvalues,2) ~= size(upperlim,2)
    ...
    if size(barvalues,1) == 1
        ...
        % EDIT AL
        upperlim = [upperlim; zeros(1,size(barvalues,2))];
        lowerlim = [lowerlim; zeros(1,size(barvalues,2))];
    ...
    for i = 1:numbars
        ...
        % modified AL: allow for upper and lower errorbars
        handles.errors(i) = errorbar(x, barvalues(:,i), lowerlim(:,i), upperlim(:,i), 'k', 'linestyle', 'none');
